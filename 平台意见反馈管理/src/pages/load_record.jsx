...
(function () {
    ...
    if (isSearch) {
        ...
        searchArgs.queryArgs = PlatformFeedbackQuery.getQueryArgs(searchParams);
        ...
        for (var i = 0; i < ids.size(); i++) {
            var objId = ids.get(i);
            var record = PlatformFeedbackService.getFeedback(objId);
            if (record) {
                listData.push(record);
            }
        }
    } else {
        totalRecords = PlatformFeedbackService.getAllFeedbackListSize();
        listData = PlatformFeedbackService.getAllFeedbackList(start, pageLimit);
    }
