...
//                newProduct.salesAmount=ProductService.getSalesAmount(productList[i].objId)||0;
        ...
        var result = {"viewAlsoView":[],"count":0};
        if(pid){

            var viewAlsoViewIds=ViewAlsoViewService.getViewAlsoView(pid);
            ...
            var viewAlsoView=productProcess(viewAlsoViewList,imgSize);
            if(viewAlsoView.length==0){
                //获取看了又看的商品列表(后台推荐)
                var commendList=commendService.getCommendObjectList(mid,pid,"historyView",count);
                viewAlsoView=productProcess(commendList,imgSize);
            }

            result["viewAlsoView"] = viewAlsoView;
            result["count"] = viewAlsoView.length;
        }
