...
//    }
    ...
    $(".right_gotop").bind({
        ...
        mouseenter:function(){$(this).children("span").show().stop(true,false).animate({"right":"35px"});},
        mouseleave:function(){$(this).children("span").animate({"right":"60px"}).hide(1);}
    });
    ...
    function scrollfix(ele) {
        if(!$(ele).length) return false;
        var item = $(ele);
        $(window).scroll(function () {
            var scrollTopp = $(this).scrollTop();
            if (scrollTopp >= 500) {
                item.addClass("fixsearch");
            } else {
                item.removeClass("fixsearch");
            }
        });
    }
    scrollfix(".fixwrap");
